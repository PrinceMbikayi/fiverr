  ...
  const actionsSwitchesBase = {
    ...
    useEffect(() => {
      if(isEmpty(actionsSwitches)) {
        const initSwitches = (scenarioId != -1)? level2Funcs.initSwitches(scenarioId,actionsSwitchesBase) : actionsSwitchesBase
    ...
    return (
      <SwitchesWrapper borderColor={theme.divider_on_body}>
              <ActionSwitch actionText={t('scenarios:ON_OFF_LIGHT')+ " "} stateValue ={actionsSwitches?.ONOFF?.value} stateName="ONOFF" callback={onSwitchToggle}/>
      </SwitchesWrapper>                   
  )
}
...
export default TypeLightActions;
// -------- styles and styled ------------------------
