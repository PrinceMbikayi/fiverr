...
export const TypePacDetails = props => {
  ...
  const TemperatureArrow = props => {
    ...
    return (
      <TouchableHighlight
        onPress={() => {
          if (callback) callback(way, carousel);
        }}
  ...
  const [currentModeInfos, setCurrentModeInfos] = useState(
    pacConfig.modes_entries[initModeIndex],
  );
    ...
    <>
      ...
      <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
        <View style={{backgroundColor: 'white'}}>
          <ModesView
            bg={selectedMode == 'heat' ? 'transparent' : 'transparent'}>
            <View
              style={{width: _modeCarouselWidth, height: _modeCarouselWidth}}>
              <Carousel
                ref={_carouselMode}
          ...
          <FansView>
            <View
              style={[
                {
                  width: _optionsCarouselWidth * 3,
                  height: _optionsCarouselWidth,
                },
          ...
          <View style={{height: 80}}>
            <TemperaturesView
              bg={selectedMode == 'heat' ? 'transparent' : 'transparent'}
              style={[showTemp == false ? {opacity: 0} : {opacity: 1}]}>
              <TemperatureArrow
                iconName="remove"
                way="previous"
              ...
              <View
                style={[
                  {
                    width: _temperatureCarouselWidth,
                    height: _temperatureCarouselWidth,
                  },
                ]}>
                <Carousel
        ...
        <ConfigurationView>
          <ConfigurationViewContent>
            {pacConfig.configurationItemIds.map((itemId, index) => {
              return (
                <ConfigurationItem key={`item-${index}-${itemId}`}>
                  <PacButton
                    disabled={!(currentModeInfos[itemId] == true)}
                    id={itemId}
