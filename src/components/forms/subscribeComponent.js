               ...
               return ( <>
                <WithTranslateFormErrors errors={errors} touched={touched} setFieldTouched={setFieldTouched}>
                 ...
                 <View style={{paddingTop:15}}>
                  <View>                   
                     <FormInput name='login' value={values.login} placeholder={t('Enter email')}  autoCapitalize='none' returnKeyType={'next'}   onChangeText = {onChangeWithRulesExternal('login',noSpace)(values,setFieldValue)}   iconColor='#2C384A'  addTestId = {emailID} bgColor={bgColor} />               
                 ...
                 {1 == 2 && <View style={{flex:1,alignItems:'flex-end',flexDirection:'row'}}>
                   <View style={{flexDirection:'row',justifyContent:'space-between',padding:15,paddingBottom:25}}>
                     ...
                     <View  style={{width:'48%'}}>
                       <AccessButton  onPress={validate} specialColor='#333' title={t("BUTTON_NEXT")} testAppium={buttonNextID}/>
                     </View>
                   </View> 
                 </View>
               }
