 ...
 * @type {React.FC<Props>}
 */
export const LineWithIcon = React.memo(props => {
  const {t, i18n} = useTranslation();
  const {title, iconRight, iconLeft, rightChevron, callback, params, color} =
    props;
  ...
  const IconRender = irProps => {
    ...
    const iconName =
      appIcons[irProps.iconName] || domusIcons[irProps.iconName]
        ? irProps.iconName + '.svg'
        : 'empty';
  ...
  return (
    <TouchableHighlight
      onPress={executeCallback}
      disabled={props.fullTouchable !== true}
      activeOpacity={activeOpacityValue}
      underlayColor={underlayColorValue}>
      <ItemRenderContainer
        dividerColor={theme.divider_on_body}
        noBorder={props.noBorder}>
        {iconLeft && <IconRender iconName={iconLeft} {...props} />}
        {props.children ? (
          <ChildrenContainer>{props.children}</ChildrenContainer>
        ) : (
          <ItemText
            color={color}
            style={[{textAlign: 'left', paddingLeft: 10}, textStyle]}>
            {props.text}
          </ItemText>
        )}
        {iconRight && <IconRender iconName={iconRight} {...props} />}
        {rightChevron && (
