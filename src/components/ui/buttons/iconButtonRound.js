...
//import Svg, {SvgCss,Circle} from 'react-native-svg';

import {SvgCss, Circle} from 'react-native-svg/css';
...
export const IconButtonRound = props => {
  ...
  const regularBackgroundColor =
    props.forceBackgroundColor || props.backgroundColor || 'transparent';
  ...
  const iconBackgroundColor =
    theme['widget--round--wrapper--color--background'] || props.backgroundColor;
  const iconWrapperColor = iconBackgroundColor;
  const iconFillColor = props.isActive
    ? theme.active_button_color || 'green'
    : iconColor;
  ...
  return (
    <View
      style={{
        width: iconSize + strokeWidth,
        height: iconSize + strokeWidth,
        alignItems: 'center',
      ...
      <View
        ...
        <Svg
          height={iconSize + 2 * strokeWidth}
          width={iconSize + 2 * strokeWidth}>
          <Circle
            cx={(iconSize + strokeWidth) / 2}
            cy={(iconSize + strokeWidth) / 2}
            r={iconSize / 2}
      ...
      <TouchableOpacity
        ...
        onPressIn={onDoPress}
        onPressOut={onDoRelease}>
        <>
          {svgr ? (
            <View style={svgrWrapperStyle}>{svgr}</View>
          ) : (
            <SvgCss
              xml={iconXml}
              width={iconSize * iconScale}
              height={iconSize * iconScale}
              fill={disabled ? 'transparent' : fillColor}
              viewBox="0 0 48 48"
