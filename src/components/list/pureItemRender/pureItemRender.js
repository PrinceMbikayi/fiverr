...
import PureItemRenderTemplate from '_components/list/pureItemRender/pureItemRenderTemplate.js';
...
const PureItemRender = ({ name, ...props }) => {
...
useEffect(()=> {
  console.log("in src > pureitemRender netInfoIsConnected",netInfoIsConnected)
},[netInfoIsConnected]);
  ...
  return (
    <PureItemRenderTemplate {...{ itemId, availableCommands, netInfoIsConnected, forceDisplay, isRoutine:isRoutine, typeDynamic: getTypeDynamic(), 'goLevel2': expandMore, 'goSettings': goSettings }} />
  )
}
...
const Exp = React.memo(PureItemRender, areEqual);
export default Exp
