...
TabButtonComponent = (props) => {
    ...
    return (
        <View style={[tabButtonComponentStyle,{borderColor:props.iconColor}, props.selected ? {borderRightWidth:0}:{borderRightWidth:1}, props.last ? {borderBottomWidth:0}:{}]}>
            <TouchableOpacity activeOpacity={0.3} underlayColor="#DDDDDD" onPress={()=>{props.callback(props.index)}}>
                <TabButton>                       
...
class ScheduleTaskPicker extends Component {
    ...
    constructor(props) {
        ...
        if(taskObject == undefined) {
            ...
            let taskDescription =  taskObject.description ||  taskObject?.statusDictionary?.__json_description;
            if(typeof taskDescription == 'string')taskDescription = JSON.parse(taskDescription)


            initTime = ScenarioHelpers.formatObjectScheduleTaskTime(taskDescription.date);
  ...
  render() {
    ...
    return (
        ...
        <View style={{minHeight:200,borderColor:"#777",borderWidth:1}}>
            <View style={{flex:1,flexDirection:'row',height:200,maxHeigh:200,}}>
                <View id="TabMenu" style={{minWidth:80,height:200}}>
                    <View style={{flex:1,flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
                        <TabButtonComponent callback={this.onSelectTab} index={0} selected ={(this.state.currentTabIndex == 0)} icon="clock" iconColor={textColor}></TabButtonComponent>
                        <TabButtonComponent callback={this.onSelectTab} index={1} selected ={(this.state.currentTabIndex == 1)} icon="sunrise" iconColor={textColor}></TabButtonComponent>
                        <TabButtonComponent callback={this.onSelectTab} index={2} selected ={(this.state.currentTabIndex == 2)} last icon="sunset" iconColor={textColor}></TabButtonComponent>                            
                    </View>
                </View>
                {this.state.isMounted &&
                <View id = "tabContents" style={{flex:2,height:200}}>
                    <View  style={[{height:"100%"}, this.state.currentTabIndex != 0 ? styles.hidden :{}]}>
                        <View style={{}}>
                            <DelayPicker initTime={this.state.initialTime} pickerId="regularTime" hideSeconds hideLabels updateCallback={this.timeSelectionCallback} />
                        </View>                        
                    </View>
