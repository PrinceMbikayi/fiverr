...
export const HeaderWithMenu = (props) => {
    ...
    const {title,noShadow,bgColor : pBgColor,hideBurger,extraButtons : options} = props;
    ...
    const showBrand = () => {
        if(theme.key == "AIRWELL") {
            return <SvgCss style={{alignSelf:'center',marginRight:10}}xml={airHomeIcons['logo']} width={34} height={34}  fill={theme.logoColor}  viewBox="0 0 48 48" preserveAspectRatio="xMinYMin slice"/>
        } else {
            return null
        }
    }
    ...
    return (
        <>
            <StyledHeaderView bgColor={bgColor || 'transparent'}>
                {!hideBurger && 
                    <TouchableWithoutFeedback onPress={openDrawer} style={{alignItems:'center'}} accessibilityLabel="drawer_toggle" {...testId}>
                        <StyledMenuBurger source={burgerImage}  />
                    </TouchableWithoutFeedback>
                }
                <StyledTitle><H1>{title}</H1></StyledTitle>
                {
                    showBrand()
                }
