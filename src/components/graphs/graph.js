...
export const Graph = (props) => {
    ...
    const axisColor = theme["card--color--text"];
    ...
    const grabDatas = async() => {
        ...
        if((isMounted && requestResult.errCode == 200) || mock == true) {
            ...
            //ok const reformatted = reformatDatas(requestResult.res);
           ...
           console.log("currentRangeType",currentRangeType)
            ...
            refill[currentRangeType] = {values : reformatted};  
    ...
    const reformatDatas = (resData) => {
        return Object.keys(resData).reduce((r,key,i) =>{
            r[key] = reformatType(resData[key]);
            return r;
        },{})
    }

    const reformatType = (typeDatas) => {
        //console.log("typeDatas",typeDatas)
        return typeDatas.reduce((r,v,i) => {
            if(v.v != null) r.push({x:new Date(v.t),y:v.v})
            return r
        },[])
    }
    ...
    const showNoDatas = () => {
        ...
        if(displayDatas && displayDatas.length == 0) {
            console.log("ben pas de données")
            return (
                ...
                <VictoryChart polar={false} height={390} containerComponent={
                    ...
                    >
                        ...
                        <VictoryAxis 
                            ...
                            style={{axis:{stroke: axisColor},tickLabels:{fill:textColor}}}
                        />
                        {/* Y axis */}
