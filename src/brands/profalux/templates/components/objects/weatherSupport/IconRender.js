...
export const IconRender = props => {
  ...
  const iconName =
    img?.indexOf('.') == -1 ? img : img?.substr(0, img?.lastIndexOf('.'));
  const objectIcon =
    useMemo(() =>
      useIcon(iconName, appIcon, imageIsStatus, activeStatusesImages),
    ) || '';
  const statusesIcons =
    useMemo(
      () => getStatusesIcons(activeStatusesImages),
      [activeStatusesImages],
    ) || [];
    ...
    <StyledIconWrapper width={iconSize} height={iconSize}>
      <>
        {img?.indexOf('.png') == -1 ? (
          ...
          <Image
            source={imgSource}
            style={{width: 24, height: 24, marginTop: 12, marginLeft: 10}}
          />
        )}
        {/* {
              isImgSource && 
        ...
        {activeStatusesImages.map((status, index) => {
          const statusColor =
            greenOn && status?.indexOf('_on') != -1 ? greenOn : fillColor;
          const statusID = buildTestId('status_' + status);
          if (status != undefined) {
            return (
              <SvgCss
                key={`status-${status}-${index}`}
                xml={statusesIcons[status]}
                width="100%"
                height="100%"
                viewBox="0 0 48 48"
                preserveAspectRatio="xMinYMin slice"
