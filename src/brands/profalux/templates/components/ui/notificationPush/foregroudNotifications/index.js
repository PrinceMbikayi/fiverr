...
import { VdpNotification } from '_components/objects/qrBasic/pushNotifications/foregroundNotification';
...
export const ForegroundNotifications = (props) => {
    ...
    useEffect(() => {  
        ...
        console.log("p_notifications",JSON.parse(JSON.stringify(p_notifications)));
       if(p_notifications) setRegularNotifications(p_notifications) 
       console.log("empty one")          
    }, [p_notifications]);

    useEffect(() => {        
     // setVdpNotification(vdp_notification) 
        console.log("--->> effect vdp_notification in src/brands/umii/templates/components/ui/notificationPush/foregroudNotifications/index.js",vdp_notification)       
  }, [vdp_notification]);
...
const doGenerateCode = () => {
    ...
    useEffect(() => {        
      // setVdpNotification(vdp_notification) 
        console.log("Umii foregroundNotifications --->> effect sav_notification",sav_notification);
        //doGenerateCode()   
  }, [sav_notification]);
    ...
    return (
        <View style={{position:'absolute',width:'100%',height:'100%',flex:1,backgroundColor:'#FFFF0000'}} pointerEvents="box-none">
          ...
          {sav_notification && <SavModal onClose={onClose}  notification={sav_notification}/>}
