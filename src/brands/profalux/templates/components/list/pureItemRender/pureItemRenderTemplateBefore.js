...
import { StyledCardView, StyledInnerCardView, StyledCardGeneralInnerWrapper, StyledSideView } from './styledBefore'
...
const PureItemRenderTemplate = (props) => {
  ...
  const HALF_WIDGET_TYPE_NAMES =  ['LightEzsp', 'SwitchEzsp', 'EzspProbe', 'NetatmoStation', 'NetatmoIndoorProbe','NetatmoOutdoorProbe','NetatmoRainGauge','NetatmoWindGauge']

  return (
    <SafeAreaView >
      <StyledCardView bodyBgColor={widgetbgColor} widthSize={HALF_WIDGET_TYPE_NAMES?.includes(typeName)? '100%' : '100%'}>
        {/* <StyledHeaderView heightPercent={20}>
                <HeaderWithBack title={uObject.name} backIcon =  "chevron-left"  objectMenu={objectMenu} noBack noShadow extraButtons={headerButtons}/>
              </StyledHeaderView> */}
        ...
        <DisconnectInnerWrapper 
          ...
          <StyledInnerCardView widthPercent={ HALF_WIDGET_TYPE_NAMES?.includes(typeName) || HALF_WIDGET_TYPE_NAMES?.includes(uniType)? 65 : 85}  bodyBgColor='transparent'>
            ...
            {((connected == false || !netInfoIsConnected) && forceDisplay != true) &&
              <>
                  {/* <DisconnectedViewOverlay bgColor={theme['card--color--deactivated-overlay']} style={{ borderTopLeftRadius:12, borderBottomLeftRadius:12,}} /> */}
          ...
          {/* (connected == false || !netInfoIsConnected)?theme['card--color--deactivated-overlay']:'transparent' */}
