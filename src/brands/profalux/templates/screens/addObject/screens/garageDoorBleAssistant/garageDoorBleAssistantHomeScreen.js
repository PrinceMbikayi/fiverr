...
import { View, SafeAreaView, Text,PermissionsAndroid,Alert, Platform, Linking, useFocusEffect } from 'react-native';
...
export const garageDoorBleAssistantHomeScreen = () => {
    ...
    const handleOnPressNextArrow = async(label) => {
        console.log('ICI_1');
        const result = await handlePersmissions();
        console.log('ICI_2');
        console.log('RESULT_BLE :', result);
        //const blePermission = result["blePermissionOk"]
        ...
        if(Platform.OS === 'ios'){
            ...
            const blePermission =  result["blePermissionOk"]
            console.log('THIS_BLOCK :', blePermission, bluetoothActivatedButton);
            if(blePermission){
