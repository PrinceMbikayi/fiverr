...
const RegisterBoxScreen = (props) => {
      ...
      const validateMe = async()=>{
            ...
            if(isValide){
                ...
                if(res.errCode == 200){
                    ...
                    //navigation.navigate('ProductsScreen',{screen:'productsScreen'})
                }else{
    ...
    const configureLater = () => {
      ...
      return (
        ...
        <TouchableOpacity 
            activeOpacity={1}
            onPress = {configureLater}
            style={{flexDirection:'row', justifyContent:'center',alignItems:'center', marginHorizontal:DEVICE_WIDTH/4, marginTop:15,backgroundColor:'transparent', borderRadius:12, minHeight:40}}>
            <Text style={{ textDecorationLine: 'underline', color:textColor, fontSize:16, fontWeight:'400'}}>{t(tns+":"+"CONFIGURED_LATER")}</Text>
