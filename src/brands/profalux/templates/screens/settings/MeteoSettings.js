...
export const MeteoSettings = (props)=>{
    ...
    const buildActionForRemoveObjectTask = (routines, weatherId)=>{

            const individualAction = routines.reduce((mArgsAcc, routineId)=>{
                const usedObject = {"name":"objectId", value:routineId}
                const usedSource = {"name":"sourceId", value:Number(weatherId)} 
                const objAction = {
                                name:"REMOVE_OBJECT_SOURCE", // REMOVE_OBJECT_SOURCE
                                mArgs:[usedObject,usedSource]
                            }
    ...
    const extractRoutineUsedByWeather = (scenarios) =>{
        const scenariosIds = scenarios.reduce((acc, item)=>{
            const uri = item?.uri
        ...
        console.log("SCENARIOS_IDS :", scenariosIds)
        return scenariosIds
    }
    ...
    const handleOKAlert = async() =>{
            ...
            const rdependencies =  weatherInfos?.rdependencies
            console.log('RDPS :', rdependencies, selection.text,selection.id);
            const scenarios = rdependencies?.scenarios || []
            const routinesIds = extractRoutineUsedByWeather(scenarios)
            
            
            if(rdependencies == undefined ){
                ...
                console.log('POINT_1');
                const myActions = buildActionForRemoveObjectTask(routinesIds, userDefaultWeather)
                ...
                await myActions.reduce(async(accumulator, action, index)=>{
                    await accumulator;
                    const removeObjectSourceResponse = await ApiObjects.createWeeklyPlanner(action).catch((err) => { console.log(err) });
                    console.log('REPONSE_SERVEUR_'+index + ":", removeObjectSourceResponse);
                    if(removeObjectSourceResponse.errCode == 200){
                        plannerId = removeObjectSourceResponse.id;
                        daysOfWeek = removeObjectSourceResponse?.res?.data?.resource?.daysOfWeek
    ...
    const create = async(name,id) => {
        ...
        if(weathers.length > 0){
            console.log('WEATHER_LIST :', weathers);
            await weathers.reduce(async (acc, idWeather, idx) => {
                    const weatherInfos = getObjectById(id)
                    console.log('INFOS_METEO :', weatherInfos);
                    await acc;
                    const result = await deleteObject(idWeather).catch((err) => { console.log(err) });
                    console.log('DELETE_WEATHER_'+idx + ":", result);
                    if(result?.errCode == 200){
                        const action = Actions.objectDelete(idWeather);    
                        dispatch(action)
                    }
                    console.log("CREATE_REQUEST_1",idx, ":",  result)
                }, Promise.resolve())
