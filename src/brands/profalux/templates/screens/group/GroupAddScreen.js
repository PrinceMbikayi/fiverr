...
const GroupAddScreen = () => {
  ...
  const compositeList = useSelector(getObjectsByTypes)["Composite"] || [];
  const compositeAndWeatherList = getGroupPossibleObject(["Composite", "Weather", "Sonde", "Netatmo", "Remote", "WeeklyPlanner","Application", ,"GarageDoor", "Gate", "ToggleGarageDoor","ToggleGate", "TriggerGate" ], objectsTypes)
  ...
  const handleSubmit = async (values) => {
    ...
    if (res.errCode == 200) {
          ...
          const grpResource = res?.res?.data?.resource
          const refresh = await refreshGroup(newGrpId,grpResource, store).catch((err) => console.log(err)); 
          console.log('REFRESH_GROUP_0 :', refresh,grpResource);
          ...
          if(modifyRes.errCode == 200){
            ...
            const action = Actions.objectDelete(newGrpId);
